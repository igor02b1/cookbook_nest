...
Neste passo, vamos criar a Classe **LocalStrategy**, que chamaremos de **local.strategy.ts**. Nesta Classe vamos implementar o **Local Strategy do Pacote Passport**, que será utilizado pelo endpoint de autenticação para validar as credenciais do usuário (usuario e senha). 
...
2. O nome do arquivo será **local.strategy.ts**. Após digitar o nome do arquivo, pressione a tecla **enter** do seu teclado para concluir. 
...
**Linhas 9, 10 e 11:** Dentro do Método Construtor da Classe, chamamos o método **super()**. A estratégia local do Passport espera apenas os Atributos **username e password**, entretanto, a Classe Auxiliar **UsuarioLogin** não possuem estes 2 Atributos, ela foi implementada com os Atributos **usuario e senha**. Para utilizar os Atributos da Classe UsuarioLogin precisamos configurar as propriedades **usernameField e passwordField**, dentro do Método **super()** para que o Passport passe a esperar pelos Atributos usuario e senha.
...
**Linha 16:** Criamos um Objeto da Classe Usuario do tipo **const**, chamado **user**, que receberá o resultado da execução do Método **validateUser(username, password)**, da Classer **AuthService**, que valida se o usuário existe e se a senha digitada está correta. Caso as 2 validações estejam corretas, será retornado um Objeto da Classe Usuario, sem a o Atributo senha, por questões de segurança e boas práticas. 
...
<div align="left"><img src="https://i.imgur.com/bQGvf3h.png" title="source: imgur.com" width="25px"/> <a href="https://github.com/rafaelq80/backend_blogpessoal_nest/blob/13_Security_JWT/blogpessoal/src/auth/strategy/local.strategy.ts" target="_blank"><b>Código fonte da Classe LocalAuthGuard</b></a>
<br /></div>
...
2. O nome do arquivo será **constants.ts**. Após digitar o nome do arquivo, pressione a tecla **enter** do seu teclado para concluir. 
...
<h2>👣 Passo 06 - Criar a Classe JwtAuthGuard</h2>
...
2. O nome do arquivo será **jwt-auth.guard.ts**. Após digitar o nome do arquivo, pressione a tecla **enter** do seu teclado para concluir. 
...
Essa Classe será utilizada em todas as **Classes Controladoras** que possuam endpoints protegidos, para habilitar a **Estratégia Jwt do Passport**, ou seja, a validação do Token JWT enviado no cabeçalho da Requisição antes de autorizar o acesso ao endpoint, mesmo que o usuário esteja autenticado.
...
<h2>👣 Passo 07 - Criar a Classe JwtStrategy</h2>

Neste passo, vamos criar a Classe **JwtStrategy**, que chamaremos de **jwt.strategy.ts**. Nesta Classe vamos implementar o **Jwt Strategy do Pacote Passport**, que será utilizado por todos os endpoints protegidos ou pela Classe Controladora para proteger todos os seus endpoints, para validar os Tokens JWT recebidos no Cabeçalho das Requisições. 
...
2. O nome do arquivo será **jwt.strategy.ts**. Após digitar o nome do arquivo, pressione a tecla **enter** do seu teclado para concluir. 
...
**Linha 8:** Cria o Método Construtor, que receberá as Injeções de Dependência. Como não precisaremos ter acesso aos Métodos de nenhuma Classe de Serviço, o Método Construtor não possui nenhuma Injeção de Dependência.

**Linha 9:** Dentro do Método Construtor da Classe, chamamos o método **super()**. Para utilizar a estratégia jwt do Passport precisamos configurar algumas propriedades. Para configurar as propriedades, assim como fizemos na Classe **LocalStrategy**, vamos utilizar o Método **super()**. 
...
**Linha 11:** Na propriedade **ignoreExpiration** definimos o seu valor como false. Isso basicamente significa que queremos bloquear solicitações com tokens expirados. Se uma chamada for feita com um token expirado, nosso aplicativo retornará **HTTP Status UNAUTHORIZED 🡪 401** (Não autorizado ).
...
Pode parecer um pouco estranho num primeiro momento o Método **validate(payload: any)** receber o payload e retornar um Objeto contendo um JSON com as propriedades do próprio payload. Para entender o porquê precisamos compreender como o **JWT Strategy** funciona na prática:
...
2. Depois de validar a assinatura, o Passport decodifica o Objeto JSON, que está dentro do Token, extraindo o **Payload**. 

3. Somente depois de concluir estes 2 processos que o Passport executará o Método **validate(payload: any)** que recebe **o JSON decodificado contendo o payload** e retorna os dados do usuario. 

Este processo garante que, se o Método **validate(payload: any)** for chamado, significa que o **JWT Token foi validado**.  
...
| <img src="https://i.imgur.com/RfjtOFi.png" title="source: imgur.com" width="100px"/> | <div align="left"> **DICA:** *A compreensão do Método validate(payload: any) ficará mais simples quando implementarmos a Classe AuthService, onde o Token JWT e o payload são gerados*.</div> |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
...
<h2>👣 Passo 06 - Registrar as Classes LocalStrategy, JwtStrategy e Importar os Módulos UsuarioModule, PassportModule e JwtModule na Classe AuthModule</h2>

Vamos registrar as **Classes LocalStrategy, JwtStrategy** e importar os **Módulos UsuarioModule, PassportModule e JwtModule**. Veja abaixo a implementação da Classe **AuthModule**:

<div align="left"><img src="https://i.imgur.com/3EozNBT.png" title="source: imgur.com" /></div>

Vamos analisar o código:

**Linha 14:** O Módulo **UsuarioModule** foi importado no array **imports**. Precisamos do Módulo UsuarioModule para validar o usuário e senha no Banco de dados da aplicação no processo de autenticação do usuário (login).

**Linha 15:** O Módulo **PassportModule** foi importado no array **imports**. Precisamos do Módulo PassportModule para implementar o pacote Passport e as suas Strategies.

**Linha 16:** O Módulo **JwtModule** foi importado no array **imports**. Precisamos do Módulo JwtModule para criar o Token JWT. Além de importar o Módulo JwtModule, será necessário executar o Método **register()** para configurar algumas propriedades.

**Linha 17:** Configura a propriedade **secret** (chave de assinatura do token JWT) com o valor da propriedade **secret da  const jwtConstants**.

**Linha 18:** Configura a propriedade **signOptions** (opções do processo de criação do token). Observe que dentro desta propriedade temos um array com as propriedades específicas do Método **sign()**, da **Classe JwtService**, do **Módulo JwtModule**. Vamos configurar a propriedade **expiresIn** (tempo de duração do Token JWT), com o valor **24h**. O Token gerado terá validade de 24 horas, após este período será necessário gerar um novo.
...
**Linha 21:** As Classes **LocalStrategy e JwtStrategy** foram registradas no array **providers**, por se tratarem de duas Classes de Serviço.
