...
3. Criar a Classe Entity Postagem
...
Primeiro vamos construir a **Classe Postagem (Model no modelo MVC)**, que será utilizada como modelo para construir a tabela **tb_postagens** (Entidade) dentro do nosso Banco de dados **db_blogpessoal**. Os campos (Atributos) da tabela serão os mesmos que estão definidos no Diagrama de Classes acima. Na próxima etapa vamos começar a construir as Classes **PostagemService** (Regras de negócio da aplicação) e **PostagemController** (Os métodos do CRUD descritos no Diagrama de Classes).

> **CRUD:** É um acrônimo para **Create** (Persistir dados na tabela tb_postagens), **Read** (Pesquisar dados na tabela tb_postagens), **Update** (Atualizar dados da tabela tb_postagens) e **Delete** (Apagar dados da tabela tb_postagens), ou seja, são os Métodos (endpoints) para interagirmos com as tabelas do nosso Banco de dados.

Depois de criar a Classe Entity Postagem, vamos executar o projeto Blog Pessoal no Nest. Após a execução veremos que será criado no MySQL, no Banco de dados **db_blogpessoal**, a tabela **tb_postagens**. Veja abaixo como ficará a estrutura da nossa tabela através do **Diagrama de Entidade e Relacionamentos (DER)** da tabela tb_postagens:
...
| Classes        | Descrição                                                    |
| -------------- | ------------------------------------------------------------ |
| **Module**     | É a Classe principal do Módulo, onde são definidas as Estruturas de dados, Relacionamentos e Dependências. As Classes Entity, Service e Controller obrigatoriamente devem ser Registradas na Classe Module, assim como a Classe Module também deve ser registrada no Módulo raiz da aplicação, na Classe **AppModule**. |
| **Entity**     | Classe responsável pela abstração dos nossos Objetos em Registros (linhas) na Tabela no Banco de dados. As Classes Entity representam os Objetos que serão persistidos no Banco de dados. As Classes Entity são utilizadas pelo TypeORM como modelo para criar automaticamente as nossas tabelas no Banco de dados. |
...
A estrutura de Módulo descrita acima é a estrutura básica. Dependendo das necessidades da aplicação, pode ser necessário gerar mais de uma Classe Entity, Service e Controller. Pode ser necessário também a implementação de Classes auxiliares, com funções específicas dentro do Module. A única Classe que será única dentro do Módulo é a Module. Para utilizar uma Classe Service ou auxiliar em outro Módulo é necessário registrar a exportação na Classe Module do Módulo.
...
| <img src="https://i.imgur.com/vVDBDG0.png" title="source: imgur.com" width="200px"/> | <div align="left"> **ALERTA DE BSM:** *Mantenha a Atenção aos Detalhes ao criar o Módulo. Um erro muito comum é criar a pasta entities fora da pasta do módulo (postagem).* </div> |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
...
Antes de implementarmos o código da Classe **PostagemModule**, vamos criar a Classe Entity **Postagem** que chamaremos de **postagem.entity.ts**.

| <img src="https://i.imgur.com/vVDBDG0.png" title="source: imgur.com" width="220px"/> | <div align="left"> **ALERTA DE BSM:** *Mantenha a Atenção aos Detalhes ao criar Classe Entity. Um erro muito comum é digitar o nome da Classe de forma incorreta. O nome da Classe deve estar dentro dos padrões do Nest para evitar erros na sua aplicação.* </div> |
| ------------------------------------------------------------ | ------------------------------------------------------------ |

1. Clique com o botão direito do mouse sobre a **pasta entities**, que foi criada dentro da pasta **postagem**, como mostra a figura abaixo, **New File** (Novo Arquivo).
...
Agora vamos criar o código da Classe **Postagem**, como mostra a figura abaixo:
 ...
 <div align="left"><img src="https://i.imgur.com/XMoek4c.png" title="source: imgur.com" /></div>

Vamos analisar o código:

**Linha 1:** Importamos o pacote **TypeORM** com os respectivos **Decorators**, utilizados na implementação da Classe Postagem.

**Linha 3:** O Decorator **@Entity** indica que esta classe define uma entidade, ou seja, ela será utilizada para gerar uma tabela no Banco de dados da aplicação. O parâmetro name, indica o nome da tabela no Banco de dados (**tb_postagens**). Caso este parâmetro não seja declarado, o TypeORM criará a tabela com o mesmo nome da Classe Entity (Postagem). Observe que o nome da Tabela segue o padrão **tb_nome_da_tabela** (tb_postagens). O prefixo **tb** indica que se trata de uma Table (Tabela). O nome da Tabela é recomendado que seja **o mesmo da Classe Entity** (postagem), em **letras minúsculas**, **sem espaços em branco ou caracteres especiais e acentos**. 

**Linhas 7, 10, 13 e 16:** foram criados os Atributos da Classe Postagem, que foram definidos no Diagrama de Classes acima. Veja na tabela abaixo a conversão de **Tipo de dados TypeScript 🡪 MySQL**
...
Observe que acima de cada atributo foram adicionadas alguns Decorators. Estes Decorators tem a função de configurar as propriedades dos atributos e criar validações para os dados que serão inseridos no Objeto (tamanho, formato e etc)

**Linhas 6:** O Decorator **@PrimaryGeneratedColumn** inidica que o Atributo será a **Chave Primária** (Primary Key - PK) da Tabela **tb_postagens** e que esta **Chave Primária** será gerada pelo Banco de dados, através da propriedade **auto-incremento** (auto-increment), que gera uma sequência numérica iniciando em 1.
...
**Nas linhas 9 e 12:** O Decorator **@Column** define que o atributo será inserido na estrutura da Tabela, ou seja, se você não declarar o Decorator **@Column** (exceto nos atributos **Chave Primária** e **Timestamp**), o atributo não será inserido na tabela. 
...
**Na linha 15:** O Decorator **@UpdateDateColumn** configura o atributo **data** como **Timestamp**, ou seja, o Nest se encarregará de obter a data e a hora do Sistema Operacional e inserir no atributo **data** toda vez que um Objeto da Classe Postagem for criado ou atualizado. 

<div align="left"><img src="https://i.imgur.com/O6PILGE.png" title="source: imgur.com" width="25px"/> <a href="https://docs.nestjs.com/techniques/database" target="_blank"><b>Documentação: <i>Banco de dados com Nest</i></b></a>

<div align="left"><img src="https://i.imgur.com/OtnA0bd.png" title="source: imgur.com" width="25px"/> <a href="https://typeorm.io/entities" target="_blank"><b>Documentação: <i>Entidades no TypeORM</i></b></a>
...
**Linha 5:** O Decorator **@Module** indica que esta classe será a Classe principal do Módulo, ou seja, nela registraremos todas as Classes (Entity, Service e Controller) criadas dentro do Módulo. 

**Linha 6:** No array **imports** vamos importar todas as **Classes Entity** (Model) do módulo, através do Método **forFeature**, da Classe **TypeOrmModule**. Veja que importamos a Classe Entity **Postagem**.

**Linha 7:** No array **providers** vamos importar todas as **Classes Service** do Módulo. Como não temos nenhuma Classe de serviço implementada, vamos deixá-lo vazio.

**Linha 8:** No array **controllers** vamos importar todas as **Classes Controller** do Módulo. Como não temos nenhuma Classe controladora implementada, vamos deixá-lo vazio.
...
**Linha 14:** No array **entities** vamos importar todas as **Classes Entity** (Model) do módulo. Insira no array a Classe **Postagem**.

| <img src="https://i.imgur.com/oScAYGc.png" title="source: imgur.com" width="150px"/> | <div align="left"> **ATENÇÃO:** *Se a Classe Entidade (Postagem em nosso exemplo), não for registrada no Módulo principal da aplicação (AppModule), a tabela (tb_postagens) não será criada no Banco de dados.* </div> |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
...
1. Na janela **Navigator**, Clique no botão <img src="https://i.imgur.com/UANRDni.png" title="source: imgur.com" /> (Atualizar)
