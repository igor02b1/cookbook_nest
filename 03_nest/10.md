...
1. Criar o método create(postagem: Postagem) para persistir uma nova Postagem no Banco de Dados na Classe PostagemService
2. Criar o método create(postagem: Postagem) para persistir uma nova Postagem no Banco de Dados na Classe PostagemController
3. Criar o método update(postagem: Postagem) para persistir uma nova Postagem no Banco de Dados na Classe PostagemService
4. Criar o método update(postagem: Postagem) para persistir uma nova Postagem no Banco de Dados na Classe PostagemController
...
-  **findByTitulo(titulo: string)** 🡪 Retorna  todos os Objetos da Classe Postagem persistidos no Banco de dados, cujo atributo titulo contenha a String enviada no parâmetro titulo do Método.   
...
**Linha 40:** Retorna a execução do Método **save()**, da Interface **postagemRepository**. O resultado da execução do Método **save()**, será um Objeto da Classe Postagem persistido (salvo) no Banco de dados, na tabela **tb_postagens**.  Observe que na instrução **return** foi inserido o comando **await** (aguarde), que na prática significa espere que a **Promise** seja resolvida (os Objetos Postagem sejam persistidos), antes de retornar a execução do Método e o valor resolvido para a Classe que acionou o Método.
...
<div align="left"><img src="https://i.imgur.com/bQGvf3h.png" title="source: imgur.com" width="25px"/> <a href="mudar" target="_blank"><b>Código fonte da Classe PostagemService</b></a>
...
<div align="left"><img src="https://i.imgur.com/B4Cs0kE.png" title="source: imgur.com" /></div>
...
<div align="left"><img src="https://i.imgur.com/bQGvf3h.png" title="source: imgur.com" width="25px"/> <a href="mudar" target="_blank"><b>Código fonte da Classe PostagemController</b></a>
...
<div align="center"><img src="https://i.imgur.com/xgLOm58.png" title="source: imgur.com" /></div>
...
**Linha 43:** Criamos o Método Assíncrono (async), chamado **update(postagem: Postagem)**, que promete retornar uma **Promise** um Objeto da Classe Postagem, que foi atualizado no Banco de dados da aplicação. 
...
Observe que dieferente do Método **create(postagem: Postagem)**, além dos Atributos titulo e texto, **o id também deve ser enviado para identificar a postagem que será atualizada**. A data não precisa ser enviada.

**Linha 45:** Criamos um Objeto da Classe Postagem, chamado buscaPostagem, para receberá o resultado da execução do Método **findById(id: number)**, da Classe **PostagemService**. O objetivo é checar se a Postagem que será atualizada existe, através da busca pelo id.  Observe que na chamada do Método **findById(id: number)** foi inserido o comando **await** (aguarde), que na prática significa espere que a **Promise** seja resolvida (o Objeto Postagem seja encontrado), antes de retornar a execução do Método e o valor resolvido para a Classe que acionou o Método.
...
**Linha 50:** Retorna a execução do Método **save()**, da Interface **postagemRepository**. O resultado da execução do Método **save()**, será um Objeto da Classe Postagem persistido (neste caso, atualizado porquê o Objeto já existe) no Banco de dados, na tabela **tb_postagens**. Observe que na instrução **return** foi inserido o comando **await** (aguarde), que na prática significa espere que a **Promise** seja resolvida (os Objetos Postagem sejam persistidos), antes de retornar a execução do Método e o valor resolvido para a Classe que acionou o Método.
...
<div align="left"><img src="https://i.imgur.com/VmuvcYL.png" title="source: imgur.com" /></div>
...
| <img src="https://i.imgur.com/hOgWvSc.png" title="source: imgur.com" width="400px"/> | <div align="left"> **ATENÇÃO:** *O Endereço do endpoint do Método update(@Body() postagem: Postagem) será igual ao endereço do Método findAll() e do Método create(@Body() postagem: Postagem). Como os verbos HTTP dos três Métodos são diferentes, não tem problema de utilizar o mesmo endereço.* </div> |
| ------------------------------------------------------------ | ------------------------------------------------------------ |

**Linha 28:** O decorator **@HttpCode(HttpStatus.OK)** indica que o Método **findById()**, terá uma **Resposta HTTP** padrão **OK 🡪 200**, ou seja, quando a Resposta da Requisição for positiva, será retornado o **HTTP Status OK 🡪 200**. Caso a Resposta seja negativa (algo deu errado), a Resposta dependerá do erro.
...
**Linha 30:** Executa o método **update(postagem: Postagem)**, **Método da Classe PostagemService**, que retornará **o Objeto da Classe Postagem** persistido (neste caso, atualizado porquê o Objeto já existe) no Banco de dados. 
...
<div align="center"><img src="https://i.imgur.com/iDYrraM.png?1" title="source: imgur.com" /></div>
...
<div align="left"><img src="https://i.imgur.com/bQGvf3h.png" title="source: imgur.com" width="25px"/> <a href="" target="_blank"><b>Código fonte do projeto</b></a>
