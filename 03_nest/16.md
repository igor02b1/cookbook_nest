...
Nesta etapa vamos começar a construir o Módulo Auth, que será utilizado para habilitar e implementar a segurança do Blog Pessoal. Através do Módulo Auth será possível realizar a autenticação dos usuários a  partir de uma tabela no Banco de dados. Para isso é necessário enviar os atributos usuario (e-mail) e a senha para que o Passport valide o usuário e gere o Token JWT, que permitirá acessar os endpoints protegidos da aplicação.
...
Nas próximas duas etapas vamos construir o Módulo Usuario e na sequência, vamos finalizar o Módulo Auth implementando o Passport e a Classe de Serviço **AuthService**, que irá nos auxiliar nos processos de autenticação (login).
...
Vamos instalar o Pacote Bcrypt através do NPM.
...
3. Para instalar o **Bcrypt**, digite o comando abaixo:
...
2. Crie a pasta **entities**. Após digitar o nome da pasta, pressione a tecla **enter** do seu teclado para concluir. 
3. Ainda na pasta **auth**, clique com o botão direito do mouse e clique na opção **New Folder** (Nova Pasta). 
4. Crie a pasta  **bcrypt**. Após digitar o nome da pasta, pressione a tecla **enter** do seu teclado para concluir. 
...
| <img src="https://i.imgur.com/vVDBDG0.png" title="source: imgur.com" width="90px"/> | <div align="left"> **ALERTA DE BSM:** *Mantenha a Atenção aos Detalhes ao criar a Classe Entidade. Um erro muito comum é criar a pasta entities fora da pasta do módulo (auth).* </div> |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
...
2. O nome do arquivo será **auth.module.ts**. Após digitar o nome do arquivo, pressione a tecla **enter** do seu teclado para concluir. 
...
2. O nome do arquivo será **usuariologin.entity.ts**. Após digitar o nome do arquivo, pressione a tecla **enter** do seu teclado para concluir. 
...
Neste passo, vamos criar a Classe Auxiliar **Bcrypt**, que chamaremos de **bcrypt.ts**. Nesta Classe vamos criar alguns Métodos que utilizarão o pacote **Bcrypt**.
...
2. O nome do arquivo será **bcrypt.ts**. Após digitar o nome do arquivo, pressione a tecla **enter** do seu teclado para concluir. 
...
<div align="center"><img src="https://i.imgur.com/nbuFvfq.png" title="source: imgur.com" /></div>
...
**Linha 9:** cria uma varável chamada **saltos**, para armazenar o numero de Saltos (salt) que serão utilizados pelo Algoritmo  **Bcrypt**, ou seja, o numero de caracteres aleatórios que serão inseridos na senha criptografada, com o objetivo de aumentar a segurança.
...
> **Hash:** É o processo de “esconder a escrita” através da aplicação de algoritmos matemáticos que modificam qualquer bloco de dados em  caracteres de comprimento fixos, chamados *hashes*. O comprimento dos dados de entrada podem ter qualquer tamanho, porém, os dados de saída sempre terão valores de *hash* de mesmo comprimento.
...
**Linha 14:** Criamos o Método Assíncrono (async), chamado **compararSenha(senhaBanco: string, senhaDigitada: string)**, que promete retornar uma **Promise** contendo um Boolean (retorna true caso a senha digitada seja igual a senha persistida no Banco de dados).

**Linha 16:** Retorna a execução do Método **compare(senhaBanco, senhaDigitada)**, da Classe **bcrypt**. Observe que a senha digitada é criptografada. Este Método retornará **true** se a senha digitada seja igual a senha persistida no Banco de dados.
...
<h2>👣 Passo 07 - Registrar a Classe UsuarioModule na Classe AppModule</h2>
...
Antes de finalizar o Módulo Auth, vamos implementar o Módulo Usuario...
