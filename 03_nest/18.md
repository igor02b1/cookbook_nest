...
O teste de software é uma forma de avaliar a qualidade da aplicação e reduzir os riscos de falhas no código ao ser colocado  em operação (Produção). Testar não se resume apenas em executar testes simples e verificar os resultados. **Executar** testes é apenas umas das atividades. Planejamento, análise, modelagem e implementação dos testes,  relatórios de progresso, resultado e avaliação da qualidade, também são partes de um **processo de testes**.
...
Outro ponto importante dos testes, que muitas vezes é ignorado, é que não basta testar apenas o melhor cenário, deve-se testar também os cenários com erros, ou seja, checar se a aplicação está fazendo todas a validações necessárias para minimizar ao máximo os erros.
...
<h3>2.1. NestJS Testing Classes</h3>
...
<table width=100%>
    ...
    <tr>
		<td><i>it/test</i></td>
        <td>São os nossos testes, cada it é um teste, logo os describes aglomeram um conjunto de its/tests;</td>
	</tr>
	<tr>
		<td><i>beforeEach</i></td>
        <td>Executa  um conjunto de ações antes de cada teste ser iniciado, para criar pré-condições necessárias para cada teste (criar Objetos, variáveis, entre outros).</td>
	</tr>
	<tr>
		<td><i>beforeAll</i></td>
        <td>Executa  um conjunto de ações, apenas uma vez, antes de iniciar a série de testes, para criar pré-condições necessárias todos os testes (criar Objetos, variáveis, entre outros).</td>
	</tr>
	<tr>
		<td><i>afterEach</i></td>
        <td>Executa  um conjunto de ações depois de cada teste ser finalizado, redefinir algumas condições após a execução de cada teste (limpar o Banco de dados, redefinir Objetos e Variáveis, entre outros).</td>
	</tr>
	<tr>
		<td><i>afterAll</i></td>
        <td>Executa  um conjunto de ações, apenas uma vez, depois de todos os testes serem finalizados, redefinir algumas condições após a execução de todos os testes (limpar o Banco de dados, redefinir Objetos e Variáveis, entre outros).</td>
	</tr>
	<tr>
		<td><i>test.skip</i></td>
...
| Método        | Descrição                                                    |
| ------------- | ------------------------------------------------------------ |
| **request()** | Cria uma nova requisição. Este método recebe como parâmetro o **Módulo de Teste**, que foi criado pela Classe **TestingModule**, que posteriormente será inicializado como uma aplicação Nest pelo Método **init()**, semelhante ao que acontece na **Classe Main()**. |
...
Vamos criar testes nos Módulos **Usuario e Auth** do Projeto Blog Pessoal, através de Requisições HTTP (semelhantes as que já foram feiras no Insomnia), de forma automatizada através do Jest e do SuperTest.
Para executarmos os testes, faremos algumas configurações pasta de testes <b>test</b>, no arquivo <b>app.e2e-spec.ts</b> e faremos a instalação da Biblioteca **SuperTest**.
